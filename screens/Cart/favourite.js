...
import ProductfavItem from './favaouritecart';
...
class Favourite extends React.Component {
  ...
  deleteRow = (rowMap, rowKey, data) => {
    ...
    const {deleteProductToFavourite} = this.props;
    deleteProductToFavourite(data);
  };
  ...
  renderItem = (data) => {
    console.log("jab favourite cart ma ay gy",data)
    return (
      <ProductfavItem
        data={data.item}
        />
  ...
  render() {
    console.log("cart ke products",this.props.FavouriteProducts)
    const { navigation } = this.props;

    if (this.props.favouriteProducts.length === 0)
    return (
      <View style={styles.blankView}>
        <Text style={styles.blankText}>{`No product on Wistlist`}</Text>
      </View>
    );
  return (
    <View style={styles.container}>
      <SwipeListView
        data={this.props.favouriteProducts}
...
const styles = StyleSheet.create({
  ...
  const mapStateToProps = (state) => ({
    favouriteProducts: selectors.getFavouriteProducts(state),
